(function(){const i="https://chromewebstore.google.com/detail/kortex/hdapplggdhndkblofffknpmnnnnbncbn";function m(){return window.location.pathname.split("/").pop()||"landing.html"}function h(n){const e=o=>n===o?"text-emerald-600":"text-slate-500 hover:text-slate-900",r=n==="landing.html"&&window.location.hash==="#pricing"?"text-emerald-600":"text-slate-500 hover:text-slate-900",t=o=>{if(typeof chrome<"u"&&chrome.runtime&&chrome.runtime.getURL)return o;const d=window.location.pathname.includes("/docs/")?"../":"./";return o==="landing.html"?d+"index.html":d+o},l=typeof chrome<"u"&&chrome.runtime&&chrome.runtime.getURL?chrome.runtime.getURL("icons/icon128.png"):"/icons/icon128.png";return`
      <header id="navbar" class="fixed left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-6xl rounded-2xl border border-slate-200/60 flex items-center justify-between px-6 bg-white/70 backdrop-blur-xl shadow-2xl shadow-slate-200/50 py-2 top-4">
        <nav class="flex items-center justify-between w-full">
          <a href="${t("landing.html")}" class="flex items-center gap-2 font-black text-xl tracking-tighter group cursor-pointer">
          ...
          <div class="hidden md:flex items-center gap-1 bg-slate-100/50 p-1 rounded-full border border-slate-200/40 text-xs font-bold">
            <a href="${t("landing.html")}" class="px-5 py-2 ${e("landing.html")} hover:bg-white rounded-full transition-all">Home</a>
            ...
            <a href="https://kortexnotebooklm.userjot.com/" target="_blank" rel="noopener" class="px-5 py-2 text-slate-500 hover:text-slate-900 hover:bg-white rounded-full transition-all">Roadmap</a>
          </div>
    ...
    `}function s(){const n=typeof chrome<"u"&&chrome.runtime&&chrome.runtime.getURL,e=document.getElementById("navbar-logo");e&&n&&(e.src=chrome.runtime.getURL("icons/icon128.png"));const a=()=>{typeof chrome<"u"&&chrome.tabs?chrome.tabs.create({url:i,active:!0}):window.open(i,"_blank")},r=()=>{if(n&&chrome.tabs){const o=chrome.runtime.getURL("src/welcome/welcome.html");chrome.tabs.create({url:o,active:!0})}else window.open(i,"_blank")},t=document.getElementById("try-free-btn"),l=document.getElementById("login-btn");t&&t.addEventListener("click",a),l&&l.addEventListener("click",r)}function c(){const n=m(),e=h(n),a=document.getElementById("navbar"),r=document.getElementById("navbar-placeholder");if(a)a.outerHTML=e;else if(r)r.outerHTML=e;else if(document.body)document.body.insertAdjacentHTML("afterbegin",e);else{document.addEventListener("DOMContentLoaded",function(){document.body.insertAdjacentHTML("afterbegin",e),s()});return}s()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",c):c()})();
